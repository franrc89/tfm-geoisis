...
public class Naturales {
	...
	@Action(semantics = SemanticsOf.SAFE)
	@MemberOrder(sequence = "1")
	@ActionLayout(named = "Buscar Naturales")
	public List<Natural> find(
			...
			final @Parameter(optionality = Optionality.OPTIONAL) @ParameterLayout(named = "Protegido") String protegido,
		...
		final Query q = pm.newQuery(Natural.class);
		...
		if (protegido != null && protegido != "") {
			sb.append("protegido.matches(\".*" + protegido + ".*\") &&");
		}
		...
		final List<Natural> results = (List<Natural>) q.execute();
